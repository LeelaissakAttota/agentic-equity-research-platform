"""Application contracts for regulatory snapshot use cases."""
...
class RegulatorySnapshotStatus(StrEnum):
    OK = "ok"
...
class RegulatorySnapshotQuery:
    company_query: CompanyQuery


@dataclass(frozen=True, slots=True)
class RegulatorySnapshotResult:
    query: RegulatorySnapshotQuery
    status: RegulatorySnapshotStatus
    message: str
    resolution: ResolutionResult | None = None
    package: CompanyRegulatoryPackage | None = None
    provider_name: str | None = None
    evaluated_at: datetime | None = None

    def __post_init__(self) -> None:
        blocked = {
            RegulatorySnapshotStatus.RESOLUTION_BLOCKED,
            RegulatorySnapshotStatus.INVALID,
            RegulatorySnapshotStatus.UNAVAILABLE,
        }
        if self.status in blocked and self.package is not None:
            msg = f"{self.status.value} results must not attach regulatory data"
...
def resolution_blocks_regulatory(status: ResolutionStatus) -> bool:
