"""Regulatory snapshot HTTP routes (Phase 5)."""
...
logger = get_logger("financial_intelligence.api.regulatory")
...
@router.get(
    ...
    response_model=RegulatorySnapshotResponse,
    responses={400: {"description": "Invalid regulatory or company query"}},
)
def regulatory_events_snapshot(
    request: Request,
...
) -> RegulatorySnapshotResponse | JSONResponse:
    container = _container(request)
    try:
        snapshot_query = RegulatorySnapshotQuery(
        ...
        return build_error_response(
            code="invalid_regulatory_query",
            message=str(exc),
            correlation_id=str(getattr(request.state, "correlation_id", "") or ""),
            status_code=status.HTTP_400_BAD_REQUEST,
        )

    result = container.get_regulatory_snapshot.execute(snapshot_query)
    logger.info(
        "regulatory_events_snapshot",
        extra={
            "regulatory_status": result.status.value,
    ...
    if result.status is RegulatorySnapshotStatus.INVALID:
        return build_error_response(
            code="invalid_regulatory_query",
            message=result.message or "invalid regulatory query",
            correlation_id=str(getattr(request.state, "correlation_id", "") or ""),
            status_code=status.HTTP_400_BAD_REQUEST,
        )
    return RegulatorySnapshotResponse.model_validate(result.to_dict())
