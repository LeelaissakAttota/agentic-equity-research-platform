...
| Workflow abuse/auto-approval | Typed transitions and explicit approval policy; MCP exposes no workflow mutation | Phase 7 suites and MCP negative tests | REST API-key authentication enforced in production/staging (Phase 11.2); rate limiting is deferred. |
